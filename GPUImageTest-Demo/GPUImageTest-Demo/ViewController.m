...
//  GPUImageTest-Demo
//
//  Created by anyongxue on 2017/4/6.
...
@property (nonatomic, strong) UIImageView *imageView;//测试照片
...
- (void)viewDidLoad {
    ...
    _imageView.frame = CGRectMake(0, (KScreenHeight - imageViewWidth) * 0.5, imageViewWidth, imageViewWidth);
...
//    _imageView.image = image;
    ...
    //高亮
    //[self ceratBightFilter];
    
    //素描
    [self ceratSketchFilter];
}


- (void)ceratBightFilter{
    ...
    //获取渲染后的图片
    UIImage *nearestNeighborImage = [brightnessFilter imageFromCurrentFramebuffer];
    
    //赋值渲染后的图片
    _imageView.image = nearestNeighborImage;
}


- (void)ceratSketchFilter{
    
    UIImage *image = [UIImage imageNamed:@"test"];
    //创建黑白素描滤镜
    GPUImageHazeFilter *sketchFilter = [[GPUImageHazeFilter alloc] init];
    //设置要渲染的区域
